<template>
  <div class="col">
    <h1 @click="sortOrder=0">정기예금 상품 목록</h1>
    <table class="table">
      <thead>
        <tr>
          <th>공시 제출일</th>
...
const depositList = computed(() => {
  return store.financeProductList.filter((element) => {
    return ((element.prdt_category === 'deposit') && (props.selectedBank === 'entireBank' ? true : element.kor_co_nm === props.selectedBank))
  })
})

const sortOrder = ref(0)
const orderedList = ref(depositList.value)

const sorting = () => {
  if (sortOrder.value === 0) {
    orderedList.value = depositList.value
  } else {
    orderedList.value = depositList.value.toSorted((a, b) => {
